// --- Macro Calculator Logic ---
let currentUnit = 'metric';
function switchUnit(unit) {
    currentUnit = unit;
    const btnMetric = document.getElementById('btnMetric');
    const btnUS = document.getElementById('btnUS');
    const metricInputs = document.getElementById('metricInputs');
    const usInputs = document.getElementById('usInputs');
    
    document.getElementById('errorMessage').style.display = 'none';
    document.getElementById('resultBox').style.display = 'none';

    if (unit === 'metric') {
        btnMetric.classList.add('active');
        btnUS.classList.remove('active');
        metricInputs.style.display = 'flex';
        usInputs.style.display = 'none';
    } else {
        btnUS.classList.add('active');
        btnMetric.classList.remove('active');
        usInputs.style.display = 'flex';
        metricInputs.style.display = 'none';
    }
}

function calculateMacros() {
    const errorMsg = document.getElementById('errorMessage');
    errorMsg.style.display = 'none';

    const age = parseFloat(document.getElementById('ageInput').value);
    const gender = document.querySelector('input[name="gender"]:checked').value;
    const activityMultiplier = parseFloat(document.getElementById('activityLevel').value);
    const goal = document.getElementById('userGoal').value;
    
    let heightCm = 0;
    let weightKg = 0;

    // Validation
    if (isNaN(age) || age < 15 || age > 100) {
        showError("Please enter a valid age (15 - 100).");
        return;
    }

    if (currentUnit === 'metric') {
        heightCm = parseFloat(document.getElementById('heightCm').value);
        weightKg = parseFloat(document.getElementById('weightKg').value);

        if (isNaN(heightCm) || isNaN(weightKg) || heightCm <= 0 || weightKg <= 0) {
            showError("Please enter valid positive numbers for height and weight.");
            return;
        }
    } else {
        const heightFt = parseFloat(document.getElementById('heightFt').value) || 0;
        const heightIn = parseFloat(document.getElementById('heightIn').value) || 0;
        const weightLbs = parseFloat(document.getElementById('weightLbs').value);

        const totalInches = (heightFt * 12) + heightIn;

        if (totalInches <= 0 || isNaN(weightLbs) || weightLbs <= 0) {
            showError("Please enter valid positive numbers for height and weight.");
            return;
        }

        heightCm = totalInches * 2.54;
        weightKg = weightLbs * 0.453592;
    }

    // 1. Calculate BMR (Mifflin-St Jeor)
    let bmr = (10 * weightKg) + (6.25 * heightCm) - (5 * age);
    if (gender === 'male') { bmr += 5; } else { bmr -= 161; }

    // 2. Calculate TDEE (Maintenance)
    let tdee = bmr * activityMultiplier;

    // 3. Adjust Calories Based on Goal
    let targetCals = tdee;
    let pPct, fPct, cPct;

    if (goal === 'lose') {
        targetCals = tdee - 500;
        // Cutting Split: Higher protein to preserve muscle
        pPct = 0.40; // 40% Protein
        fPct = 0.30; // 30% Fat
        cPct = 0.30; // 30% Carbs
    } else if (goal === 'gain') {
        targetCals = tdee + 500;
        // Bulking Split: High carbs for energy surplus
        pPct = 0.30; // 30% Protein
        fPct = 0.20; // 20% Fat
        cPct = 0.50; // 50% Carbs
    } else {
        // Maintain
        pPct = 0.30; // 30% Protein
        fPct = 0.35; // 35% Fat
        cPct = 0.35; // 35% Carbs
    }

    // Safety: Ensure target calories don't drop to dangerous levels
    if (gender === 'female' && targetCals < 1200) targetCals = 1200;
    if (gender === 'male' && targetCals < 1500) targetCals = 1500;

    // 4. Convert Calories to Grams (Protein=4cal/g, Fat=9cal/g, Carbs=4cal/g)
    const proteinGrams = (targetCals * pPct) / 4;
    const fatGrams = (targetCals * fPct) / 9;
    const carbGrams = (targetCals * cPct) / 4;

    displayMacros(targetCals, proteinGrams, fatGrams, carbGrams, pPct, fPct, cPct);
}

function showError(msg) {
    const errorMsg = document.getElementById('errorMessage');
    errorMsg.innerText = msg;
    errorMsg.style.display = 'block';
    document.getElementById('resultBox').style.display = 'none';
}

function displayMacros(cals, protein, fat, carbs, pPct, fPct, cPct) {
    document.getElementById('targetCals').innerText = Math.round(cals).toLocaleString();
    
    document.getElementById('proteinGrams').innerText = Math.round(protein);
    document.getElementById('fatGrams').innerText = Math.round(fat);
    document.getElementById('carbGrams').innerText = Math.round(carbs);

    document.getElementById('proteinPct').innerText = `${pPct * 100}%`;
    document.getElementById('fatPct').innerText = `${fPct * 100}%`;
    document.getElementById('carbPct').innerText = `${cPct * 100}%`;

    document.getElementById('resultBox').style.display = 'block';
}

// --- Global UI Logic (Dark Mode & Mobile Menu) ---
document.addEventListener('DOMContentLoaded', () => {
    const themeToggleBtn = document.getElementById('theme-toggle');
    const body = document.body;
    
    if(themeToggleBtn) {
        const themeIcon = themeToggleBtn.querySelector('i');
        if (localStorage.getItem('theme') === 'dark') {
            body.classList.add('dark-mode');
            themeIcon.classList.replace('fa-moon', 'fa-sun');
        }

        themeToggleBtn.addEventListener('click', () => {
            body.classList.toggle('dark-mode');
            if (body.classList.contains('dark-mode')) {
                themeIcon.classList.replace('fa-moon', 'fa-sun');
                localStorage.setItem('theme', 'dark');
            } else {
                themeIcon.classList.replace('fa-sun', 'fa-moon');
                localStorage.setItem('theme', 'light');
            }
        });
    }

    const mobileMenuBtn = document.querySelector('.mobile-menu-btn');
    const desktopNav = document.querySelector('.desktop-nav');

    if(mobileMenuBtn && desktopNav) {
        mobileMenuBtn.addEventListener('click', () => {
            desktopNav.classList.toggle('active');
        });
    }
});
